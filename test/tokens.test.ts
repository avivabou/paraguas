...
describe('splitWithTokens exclude', () => {
    it('leaves basic HTML tags as literal text', () => {
        expect(splitWithTokens('Choose <i>mandatory</i> now', { i: (l) => `!${l}!` })).toEqual([
            'Choose <i>mandatory</i> now',
        ]);
    });
});
